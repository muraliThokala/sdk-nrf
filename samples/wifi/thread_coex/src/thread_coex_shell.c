...
static struct sockaddr_in addr_my_in = {
	.sin_family = AF_INET,
	.sin_port = htons(5001),
};
			 ...
			 const char *argv[])
{
	...
	if (argc < 4) {
		printk("invalid # of args : %d\n", argc);
		printk("Usage: coex_config_pta wifi_band is_sep_antennas ");
		printk("is_sr_ble\n\n");
	...
	if (result) {
		printk("Configuration of PTA - FAIL\n");
	} else {
		printk("Configuration of PTA - SUCCESS\n");
	}
			...
			char **argv)
{
	...
	if (status) {
		printk("COEXC disable - FAIL\n");
	} else {
		printk("COEXC disable - SUCCESS\n");
	}
	return status;
}
			...
			char **argv)
{
	...
	if (argc < 2) {
		printk("invalid # of args : %d\n", argc);
		printk("Usage: config_sr_switch is_sep_antennas\n\n");
		printk("       is_sep_antennas: 0 for shared antenna, 1 for separate antennas\n");
		return -ENOEXEC;
	}
	...
	if (ret != 0) {
		printk("Configuration of SR side switch - FAIL\n");
	} else {
		printk("Configuration of SR side switch - SUCCESS\n");
	}
	return ret;
}
			...
			char **argv)
{
	...
	if (argc < 8) {
		printk("invalid # of args : %d\n", argc);
		printk("Usage: wifi_run_tput protocol direction peerIP ");
		printk("destPort duration pktSize baudrate\n\n");
	...
	parse_ipv4_addr(argv[3], &addr_my_in);
	params.duration_ms = duration_ms * 1000; /* sec to ms */
	params.rate_kbps = rate_kbps*1000;
	params.packet_size = packet_size * 1024;

	net_sprint_ipv4_addr(&addr_my_in.sin_addr);
	addr_my_in.sin_port = htons(port);

	memcpy(&params.peer_addr, &addr_my_in, sizeof(addr_my_in));
	...
	if (ret != 0) {
		printk("Running Wi-Fi throughput - FAIL\n");
	}
	return ret;
}
