...
/** @file
 * @brief Wi-Fi and openThread coexistence test functions
 */
		...
		bool is_ot_client, bool is_wifi_conn_scan)
{
	...
	if (is_ot_client) {
		if (is_wifi_conn_scan) {
			...
			LOG_INF("Wi-Fi connected scan, openThread client");
		} else {
			LOG_INF("Test case: wifi_scan_ot_discov");
			LOG_INF("Wi-Fi scan, openThread client");
		}
	} else {
		if (is_wifi_conn_scan) {
			LOG_INF("Test case: wifi_scan_ot_discov");
			LOG_INF("Wi-Fi connected scan, openThread server");
		} else {
			LOG_INF("Test case: wifi_scan_ot_discov");
			LOG_INF("Wi-Fi scan,  openThread server");
		}
	}
...
int wifi_scan_ot_tput(bool is_ant_mode_sep, bool test_openThread, bool test_wifi,
			bool is_ot_client, bool is_wifi_conn_scan)
{
	int ret = 0;
	uint64_t test_start_time = 0;
...
int wifi_con_ot_discov(bool test_wifi, bool is_ant_mode_sep,	bool test_openThread, bool is_ot_client)
{
	int ret = 0;
	uint64_t test_start_time = 0;
...
int wifi_con_ot_tput(bool test_wifi, bool is_ant_mode_sep,	bool test_openThread, bool is_ot_client)
{
	int ret = 0;
	uint64_t test_start_time = 0;
	...
	if (is_ot_client) {
		if (is_wifi_server) {
			if (is_wifi_zperf_udp) {
				...
				LOG_INF("openThread client, Wi-Fi UDP server");
			} else {
				LOG_INF("openThread client, Wi-Fi TCP server");
			}
		} else {
			if (is_wifi_zperf_udp) {
				LOG_INF("Test case: wifi_tput_ot_discov");
				LOG_INF("openThread client, Wi-Fi UDP client");
			} else {
				LOG_INF("openThread client, Wi-Fi TCP client");
			}
		}
	} else {
		if (is_wifi_server) {
			if (is_wifi_zperf_udp) {
				LOG_INF("Test case: wifi_tput_ot_discov");
				LOG_INF("openThread server, Wi-Fi UDP server");
			} else {
				LOG_INF("openThread server, Wi-Fi TCP server");
			}
		} else {
			if (is_wifi_zperf_udp) {
				LOG_INF("Test case: wifi_tput_ot_discov");
				LOG_INF("openThread server, Wi-Fi UDP client");
			} else {
				LOG_INF("openThread server, Wi-Fi TCP client");
			}
		}
	}
...
int wifi_con_stability_ot_discov_interference(bool test_wifi, bool test_openThread, bool is_ot_client,
...
int wifi_con_stability_ot_tput_interference(bool test_wifi, bool is_ant_mode_sep, bool test_openThread,
		bool is_ot_client)
{
	int ret = 0;
	uint64_t test_start_time = 0;
