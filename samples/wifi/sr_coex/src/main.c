...
#define CONFIG_TEST_ID1
//#define CONFIG_TEST_ID2
//#define CONFIG_TEST_ID3
//#define CONFIG_TEST_ID4
//#define CONFIG_TEST_ID5
...
static int wifi_tput_ble_tput(bool test_wlan, bool wifi_coex_enable,
				bool antenna_mode,
...
static void disconnect_ble(bool test_ble, bool ble_role)
{
	/* Don't do BLE disconnection if SPW allocated for WLAN */
	if (test_ble) {
...
int main(void)
{
	...
	#if 0 /* this is not required because SR grant is active low */
	...
	if (!(IS_ENABLED(CONFIG_RPU_ENABLE))) {
		...
		ret = wifi_tput_ble_tput(test_wlan, wifi_coex_enable, antenna_mode,
				test_ble, ble_role, coex_hardware_enable);
		if (ret != 0) {
			LOG_ERR("Running Wi-Fi and BLE throughput fail");
			goto err;
		}
	#endif 
		...
		if (ret != 0) {
			LOG_ERR("Running Wi-Fi throughput and BLE scan/adv fail");
			goto err;
		}
	#endif 
