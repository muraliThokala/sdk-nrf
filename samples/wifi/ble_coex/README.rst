...
| -only         |              | nRF5340 DK and select role set to peripheral.                  |
...
|               |              | configuration. Bluetooth LE connection happens in this process.|
|               |              | Run the Bluetooth LE throughput on the DUT using the command   |
|               |              | ``bt_run_tput`` with role set to central.                      |
+---------------+--------------+----------------------------------------------------------------+
| Wi-Fi and     | Disabled/    | Run Wi-Fi iPerf using the command "iperf -s -i 1 -u" on the    |
| Bluetooth LE  | Enabled      | test PC. This runs Wi-Fi iperf in server mode.                 |
...
|               |              | command ``wifi_run_tput``.                                     |
|               |              | Run the Bluetooth LE throughput on the DUT using the command   |
|               |              | ``bt_run_tput`` with role set to central and wait for the      |
|               |              | results.
|               |              |                                                                |
