...
const uint32_t ch_config_sep_ble[] = {
	0x00000028, 0x00000000, 0x001e1023, 0x00000000, 0x00000000,
	0x00000000, 0x00000021, 0x000002ca, 0x00000055
};
...
int nrf_wifi_coex_config_non_pta(bool separate_antennas, bool is_sr_protocol_ble,
								bool is_ch_enable, bool is_rf_switch_to_wifi)
{
	...
	num_reg_to_config = ((EXT_SYS_WLANSYSCOEX_CH_SW_CTRL_MAP -
	...
	params.config_ch_enable = 1;
	/* used only when config_ch_enable=1 */
	params.is_ch_enable = is_ch_enable;
	/* used only when config_ch_enable=1 */
	params.is_rf_switch_to_wifi = is_rf_switch_to_wifi;

	/* set to 1 to indicate the CH is to be configured for non-PTA, 0 otherwise */
	params.config_non_pta = 1; 

	/* set to 1 to indicate the CH is to be configured for PTA, 0 otherwise */
	params.config_pta = 0;
	/* valid when config_pta = 1; 1 indicates Wi-Fi for 2.4G,0 otherwise.*/
	params.is_pta_config_for_2pt4g = 0;

	params.num_reg_to_config = num_reg_to_config | 
		(params.config_ch_enable << CONFIG_CH_ENABLE_SHIFT) |
		(params.is_ch_enable << IS_CH_ENABLE_SHIFT) |
		(params.is_rf_switch_to_wifi << IS_RF_SWITCH_TO_WIFI_SHIFT) |
		(params.config_non_pta << CONFIG_NON_PTA_SHIFT) |
		(params.config_pta << CONFIG_PTA_SHIFT) |
		(params.is_pta_config_for_2pt4g << IS_PTA_CONFIG_FOR_2PT4G_SHIFT);
	...
	bool is_sr_protocol_ble)
{
	...
	uint32_t num_reg_to_config = 1;
	uint32_t cmd_len, index;
	uint32_t is_pta_config_for_2pt4g = 0;
	...
	params.message_id = HW_CONFIGURATION;
	params.config_ch_enable = 0; 
	params.is_ch_enable = 0;
	params.is_rf_switch_to_wifi = 0;
	params.config_non_pta = 0; 
	params.config_pta = 1;
	params.is_pta_config_for_2pt4g = is_pta_config_for_2pt4g;
