...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
    ...
    //}
	//
	///* Do NOT reconnect if the test is over */
    //if (exit_llpm_test) {
    //    return;
    //}
...
static int enable_qos_conn_evt_report(void)
{
	...
	#ifdef CONFIG_BLE_DEVICE_ROLE_CENTRAL
	printk("Choose the device role as peripheral on the peer device\n");
	#else
	printk("Choose the device role as central on the peer device\n");
	#endif
...
static void test_run(void)
{
	...
	if (!test_ready || exit_llpm_test || !default_conn) {
        return;
    }
	...
	uint64_t init_stamp = k_uptime_get();
	//uint32_t num_iterations = 0;

	printk("initial stamp = %llu\n", init_stamp);

	while (true) {
		
		if (exit_llpm_test || !default_conn) {
            break;
        }
		...
		//num_iterations++;
		//printk("num_iterations = %u\n", num_iterations);

		uint64_t updated_stamp = k_uptime_get();
		uint64_t time_stamp_diff = updated_stamp - init_stamp;

		//printk("updated_stamp = %llu\n", updated_stamp);
		//printk("time_stamp_diff = %llu\n", time_stamp_diff);

		if (time_stamp_diff > CONFIG_LLPM_TEST_DURATION) {
			exit_llpm_test = true;
			break;
		}
	}
...
int llpm_init(void)
{
	...
	#ifdef CONFIG_BLE_DEVICE_ROLE_CENTRAL
		printk("=======================\n");
		printk("Device role: Central\n");
		printk("=======================\n");
		scan_init();
		scan_start();
	#else
		printk("=======================\n");
		printk("Device role: Peripheral\n");
		printk("=======================\n");
		adv_start();
	#endif
...
int llpm_start(void)
{
	for (;;) {
		...
		test_run();
		if (exit_llpm_test == true){
			//printk("\nExiting the LLPM test \n");
			
			/* Prevent disconnected() from restarting scan */
			bt_scan_stop();
