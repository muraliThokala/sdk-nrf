...
int nrf_wifi_coex_config_gpio_ctrl(unsigned int alt_swctrl1, unsigned int invert_bt_coex_grant)
{
    
    enum nrf_wifi_status status = NRF_WIFI_STATUS_FAIL;
	
	status = nrf_wifi_fmac_conf_gpio_ctrl_sr_coex(rpu_ctx->rpu_ctx,
				alt_swctrl1,
				invert_bt_coex_grant);
	
	//int status = 0;
	//unsigned int abs_sys_sleep_ctrl_gpio_ctrl = 0xA4002DC8;
	////unsigned int alt_swctrl1 = 0;
	////unsigned int invert_bt_coex_grant = 0;
	//
	//unsigned int sleep_ctrl_gpio_ctrl_read = 0;
    //unsigned int sleep_ctrl_gpio_ctrl_write = 0;
    //unsigned int alt_swctrl1_function_bt_coex_status1_mask = 0x00000040UL;
	//unsigned int alt_swctrl1_function_bt_coex_status1_shift = 6;	
	//unsigned int invert_bt_coex_grant_output_mask = 0x00000200L;
	//unsigned int invert_bt_coex_grant_output_shift = 9;
	//
    //status = hal_rpu_reg_read(fmac_dev_ctx->hal_dev_ctx, 
	//			&sleep_ctrl_gpio_ctrl_read, abs_sys_sleep_ctrl_gpio_ctrl);
    //    
	...
	//sleep_ctrl_gpio_ctrl_write |= ((alt_swctrl1 << alt_swctrl1_function_bt_coex_status1_shift) |
	//								(invert_bt_coex_grant << invert_bt_coex_grant_output_shift));
    //
    //hal_rpu_reg_write(fmac_dev_ctx->hal_dev_ctx,
	//	abs_sys_sleep_ctrl_gpio_ctrl, sleep_ctrl_gpio_ctrl_write);
	//
    //status = hal_rpu_reg_read(fmac_dev_ctx->hal_dev_ctx,
	//	&sleep_ctrl_gpio_ctrl_read, abs_sys_sleep_ctrl_gpio_ctrl);
    ///* nrf_wifi_osal_log_info("alt_swctrl1 = %d, invert_bt_coex_grant = %d, sleep_ctrl_gpio_ctrl=0x%x",
	//	alt_swctrl1, invert_bt_coex_grant, sleep_ctrl_gpio_ctrl_read); */
