 ...
 *
 * Order matters. Coexistence only runs when both radios are up, so the gate
 * stays shut until the second of these two calls.
 */
...
static int tb_control_shared_lna(void)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(lna_modes); i++) {
		...
		ret = coex_tb_params_coex_blob_lna(lna_modes[i], blob, sizeof(blob), &blob_len,
						   &original);
		if (ret != 0) {
			printk("Coex TB: LNA blob build failed (%d)\n", ret);
			return ret;
		}
...
static int tb_sr_activity_info(void)
{
	...
	if (ret != 0) {
		printk("Coex TB: SR activity start failed (%d)\n", ret);
		return ret;
	}
	...
	ret = coex_cd_update_short_range_activity_info(&activity);
	if (ret != 0) {
		printk("Coex TB: SR activity end failed (%d)\n", ret);
		return ret;
	}
...
static int tb_get_coex_manager_stats(void)
{
	...
	if (ret != 0) {
		printk("Coex TB: get stats failed (%d)\n", ret);
		return ret;
	}
...
static const struct tb_step tb_steps[] = {
	...
	{"configure COEXC", tb_configure_enable_coexc},
#endif
...
int coex_tb_run_all(void)
{
	...
	if (ret == 0) {
		...
		printk("\n========== nRF71 SR coexistence driver test bench FAIL: %s (%d) =====\n",
		       (failed_step != NULL) ? failed_step : "unknown step", ret);
	}
