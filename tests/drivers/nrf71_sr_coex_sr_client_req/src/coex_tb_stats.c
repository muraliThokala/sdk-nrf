...
static void coex_tb_print_patch_stats(const struct cm_fsm_patch_stats_sr_t *p)
{
	...
	printk("  cmd_sr_sw_client_req_cnt_patch:     %u\n", p->cmd_sr_sw_client_req_cnt_patch);
}
				...
				const struct cm_fsm_patch_stats_sr_t *patch_stats)
{
	...
	if (patch_stats != NULL) {
		...
		printk("  sr_sw_client_event_to_host_cnt:    %u\n",
		       patch_stats->sr_sw_client_event_to_host_cnt);
				      ...
				      const struct cm_fsm_patch_stats_sr_t *patch_stats)
{
	...
	if (patch_stats != NULL) {
		...
		printk("\n==================== Patch Stats Checks ====================\n");
		printk("  SR SW client patch cmd count:     %u\n",
		       patch_stats->cmd_sr_sw_client_req_cnt_patch);
		printk("  SR SW client patch event count:   %u\n",
		       patch_stats->sr_sw_client_event_to_host_cnt);
	} else {
