 ...
 * (request and release).
 */
static int tb_sr_sw_client_request(void)
{
	struct coex_wifi_priority_range_t wifi_range;
	struct coex_sr_priority_range_t sr_range;
	struct coex_user_params_t user_params;
	struct coex_sr_sw_client_params_t sw_params;
	enum coex_sr_sw_client_req_status_t grant_status;
	int ret;

	printk("\n============================================= SR SW client request/release\n");

	coex_tb_wait_ms(COEX_TB_STATS_POLL_MS);
	...
	coex_tb_params_user_params_sr_sw_client_test(&user_params);

	for (uint32_t iter = 1U; iter <= COEX_TB_NUM_SR_SW_CLIENT_TESTS; iter++) {
		printk("--- SR SW client iteration %u ---\n", iter);
		...
		printk("--- Post SR SW client REQUEST ---\n");
		coex_tb_params_sr_sw_client(&sw_params, SR_SW_CLIENT_REQUEST, 30U * 1000U,
					    SR_BAND_2PT4G);
		ret = coex_cd_sr_software_client_request(&sw_params, &grant_status);
		if (ret != 0) {
			return ret;
		}

		printk("Coex TB: SR SW client REQUEST grant/status=%u\n", grant_status);

		coex_tb_wait_ms(COEX_TB_STATS_POLL_MS);

		printk("--- Post SR SW client RELEASE ---\n");
		coex_tb_params_sr_sw_client(&sw_params, SR_SW_CLIENT_RELEASE, 0U, SR_BAND_2PT4G);
		ret = coex_cd_sr_software_client_request(&sw_params, &grant_status);
		if (ret != 0) {
			return ret;
		}

		printk("Coex TB: SR SW client RELEASE grant/status=%u\n", grant_status);
	}
...
#ifdef COEX_TB_TEST_SR_SW_CLIENT_REQUEST
	if (ret == 0) {
		ret = tb_sr_sw_client_request();
	}
#endif
