...
void coex_tb_params_user_params_sr_sw_client_test(struct coex_user_params_t *user_params)
{
	if (user_params == NULL) {
		return;
	}
	...
	/*
	 * Mirror firmware TB: Wi-Fi client protection at 0 % for SR SW client tests.
	 * SR-specific user params (sr_conn_prot_prob, etc.) are not in the ROM 1.0
	 * host @c coex_user_params_t yet; firmware applies SR defaults internally.
	 */
	coex_tb_params_user_params_wifi_prob(user_params, 0U);
}
...
void coex_tb_params_sr_sw_client(struct coex_sr_sw_client_params_t *params,
				  enum coex_sr_sw_client_req_type_t req_type,
				  uint32_t timeout_ms,
				  enum sr_rf_freq_band_t band)
{
	if (params == NULL) {
		return;
	}

	*params = (struct coex_sr_sw_client_params_t){
		.sw_client_request = req_type,
		.sw_client_pti_level = SR_SW_CLIENT_REQ_PTI_LOW,
		.sw_client_type = SR_CONNECTION,
		.request_timeout_in_ms = timeout_ms,
		.sr_operating_band = band,
	};
}
